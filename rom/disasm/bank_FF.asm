...
C - - - - - 0x00146A 00:D45A: 18        CLC
C - - - - - 0x00146B 00:D45B: 65 0B     ADC ram_frm_cnt_lo
C - - - - - 0x00146D 00:D45D: EA        NOP
C - - - - - 0x00146E 00:D45E: EA        NOP
C - - - - - 0x00146F 00:D45F: EA        NOP
...
; PvP: вместо очереди по таймеру — проверяем per-player запросы респавна.
; JMP(3б)+NOP*4 сохраняют размер 7 байт -> адреса не сдвигаются.
C - - - - - 0x001B58 00:DB48: 4C xx xx  JMP sub_DB48_patched
C - - - - - 0x001B5B 00:DB4B: EA        NOP
C - - - - - 0x001B5C 00:DB4C: EA        NOP
C - - - - - 0x001B5D 00:DB4D: EA        NOP
C - - - - - 0x001B5E 00:DB4E: EA        NOP
...
bra_DDD4_enemy:
; PvP: направление врага (X>=2) берём из сетевой зоны ram_net_enemy_dir вместо RNG.
; JMP(3б)+NOP*4 сохраняют исходный размер 7 байт -> последующий код не сдвигается.
C - - - - - 0x001DE4 00:DDD4: 4C xx xx  JMP sub_net_enemy_dir
C - - - - - 0x001DE7 00:DDD7: EA        NOP
C - - - - - 0x001DE8 00:DDD8: EA        NOP
C - - - - - 0x001DE9 00:DDD9: EA        NOP
C - - - - - 0x001DEA 00:DDDA: EA        NOP
loc_DDDB:
...
sub_DE72:
; PvP: если игрок держит направление для этого врага (ram_net_enemy_dir != FF) —
; сетевое управление; иначе — оригинальный AI (продолжение на DE75).
C - - - - - 0x001E82 00:DE72: 4C xx xx  JMP sub_DE72_patched
; / 04
...
C - - - - - 0x001E94 00:DE84: 4C xx xx  JMP sub_net_enemy_dir_store
C - - - - - 0x001E97 00:DE87: EA        NOP
C - - - - - 0x001E98 00:DE88: EA        NOP
...
C - - - - - 0x002181 00:E171: 20 xx xx  JSR sub_net_enemy_fire_check
C - - - - - 0x002184 00:E174: EA        NOP
C - - - - - 0x002185 00:E175: EA        NOP
...
; bzk garbage
; PvP: подпрограмма сетевого направления ATT-танка.
; X = индекс танка (2..7). Возвращает A = tank_flag (A0|dir) если в сети нажато
; направление, иначе ОРИГИНАЛЬНЫЙ AI (sub_DDA2: флаг наведения на цель, как в
; ROM). Раньше возвращался текущий флаг — танк не перенацеливался и застревал.
; Детерминированно.
sub_net_enemy_dir:
    TXA
    SEC
    SBC #$02
    TAY
    LDA ram_net_enemy_dir,Y
    BMI bra_net_keep_dir
    ORA #con_tank_flag_A0
    RTS
bra_net_keep_dir:
    ; Нет сетевого ввода -> оригинальный AI: продолжить выбор флага из sub_DDA2.
    ; bra_DDE4 = tbl_E486[ram_0064] — точное направление к цели (база/игрок).
    JMP bra_DDE4
; PvP: вариант с сохранением результата (для sub_DE72 и прочих точек AI-поворотов).
sub_net_enemy_dir_store:
    TXA
    SEC
    SBC #$02
    TAY
    LDA ram_net_enemy_dir,Y
    BMI bra_net_store_keep
    ORA #con_tank_flag_A0
    STA ram_tank_flags,X
    RTS
bra_net_store_keep:
    RTS
; PvP: per-player respawn — сканирует слоты врагов 2..7 и спавнит слот, если
; игрок запросил респавн (ram_net_enemy_respawn[X-2]). Детерминированно.
sub_net_respawn_check:
    LDX #$07
bra_rs_loop:
    CPX #$02
    BCC bra_rs_done
    LDA ram_tank_flags,X
    BNE bra_rs_next
    TXA
    SEC
    SBC #$02
    TAY
    LDA ram_net_enemy_respawn,Y
    BEQ bra_rs_next
    JSR sub_E363_tank_spawn_handler
    LDA #$00
    STA ram_net_enemy_respawn,Y
    RTS
bra_rs_next:
    DEX
    JMP bra_rs_loop
bra_rs_done:
    RTS
; PvP: обёртка над исходным спавном: сначала проверка респавна, затем таймер.
sub_DB48_patched:
    JSR sub_net_respawn_check
    LDA ram_enemy_timer_before_spawn
    BEQ bra_rs_db4f
    DEC ram_enemy_timer_before_spawn
    RTS
bra_rs_db4f:
    JMP bra_DB4F_time_to_spawn
; PvP: огонь врага — если игрок нажал A (ram_net_enemy_fire[X-2]=1), вернуть 0
; (=> BNE не взят => выстрел), иначе ОРИГИНАЛЬНЫЙ RNG-выстрел (как в ROM E171:
; JSR sub_D44D; AND #$1F; BNE — шанс 1/32). Раньше возвращал 1 (пропуск) — враг
; вообще не стрелял без сетевого ввода.
sub_net_enemy_fire_check:
    TXA
    SEC
    SBC #$02
    TAY
    LDA ram_net_enemy_fire,Y
    BNE bra_ef_fire
    ; Нет сетевого огня -> оригинальный RNG. JMP (а не JSR): sub_D44D сам делает
    ; RTS, возвращаясь в E174 (AND #$1F; BNE) — точь-в-точь как исходный вызов.
    JMP sub_D44D_generate_random_number
bra_ef_fire:
    LDA #$00
    RTS
- - - - - - 0x002FE0 00:EFD0: FF        .byte $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF   ; 
; PvP: вход sub_DE72 — если игрок держит направление, сетевое управление, иначе AI.
sub_DE72_patched:
    TXA
    SEC
    SBC #$02
    ...
    RTS
bra_de72_orig:
    LDA ram_enemy_spawn_interval
    LSR
    JMP $DE75
- - - - - - 0x003000 00:EFF0: FF        .byte $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF   ; 
