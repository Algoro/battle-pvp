...
import { MultiTransport } from "../transport/multi.ts";
import { encodeFrameBatch } from "../protocol/frame.ts";
import type { Input, Transport } from "../ports.ts";
...
class SelectiveDrop implements Transport {
  ...
  dropAt: number;
  count = 0;
  constructor(inner: Transport, dropAt: number) {
    this.inner = inner;
    this.dropAt = dropAt;
  }
...
function drive(
  ...
  tb: LocalEndpoint,
  frames: number,
  makeInput: (side: string, f: number) => Input[],
): void {
...
test("избыточность ввода: потерянный пакет восстанавливается без desync", () => {
  ...
  const mi = (side: string, f: number): Input[] => [{ port: side === "A" ? 0 : 2, buttons: (f * 7 + (side === "A" ? 1 : 2)) & 0x0f }];
...
test("MultiTransport: три клиента (2v2/N) сходятся через мультиплекс", () => {
  ...
  const ch: (LocalEndpoint | null)[][] = Array.from({ length: n }, () => new Array(n).fill(null));
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
  ...
  const events: SessionEvent[][] = Array.from({ length: n }, () => []);
  const sessions: RollbackSession[] = [];
  for (let i = 0; i < n; i++) {
    const mt = new MultiTransport(ch[i].filter(Boolean) as LocalEndpoint[]);
    const myId = "p" + i;
    const others = [];
...
test("аудио: откат глушит звук на время переигровки", () => {
  ...
  const track = (romBytes: Uint8Array) => {
    ...
    g.setAudioSuppressed = (v: boolean) => { st.suppressed = !!v; if (v) st.suppressions++; orig(v); };
    return { g, st };
  };
  ...
  for (const [name, st] of [["A", A.st], ["B", B.st]] as const) {
