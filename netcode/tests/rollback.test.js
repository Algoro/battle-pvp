...
function makePeerInputs(seed, frames, ports) {
  let s = seed >>> 0;
...
function runMatch({ delayA = 0, delayB = 0, lossA = 0, lossB = 0, jitter = 0, frames = 200, checkpointInterval } = {}) {
  const rom = loadRom();
...
test("детект десинка: расхождение состояния фиксируется сверкой хэша", () => {
  ...
  for (let f = 0; f < 90; f++) {
    ...
    sessA.transport.flush();
    sessB.transport.flush();
  }
