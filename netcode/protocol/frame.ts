...
export type DecodedFrameBatch = { frames: InputEntry[]; headFrame: number };
export type DecodedHashCheck = { frame: number; hash: string };
export type DecodedPing = { seq: number; t: number };
export type DecodedPong = { seq: number; t: number };
export type DecodedSnapshotRequest = { frame: number };
export type DecodedSnapshotChunk = {
  frame: number;
  hash: string;
  seq: number;
  total: number;
...
function writeInputEntries(buf: Uint8Array, off: number, inputs: Input[]): number {
...
export function decodeFrame(buf: Uint8Array | null | undefined): DecodedFrame | null {
...
export function decodeFrameBatch(buf: Uint8Array | null | undefined): DecodedFrameBatch | null {
  if (!buf || buf.length < BATCH_HEADER || buf[0] !== PKT.BATCH) return null;
  const dv = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
  const headFrame = dv.getUint32(1, true);
  const count = buf[5];
  let off = BATCH_HEADER;
  const frames: InputEntry[] = [];
...
// --- HASH ---
export function encodeHashCheck(frame: number, hash: string): Uint8Array {
...
export function decodeHashCheck(buf: Uint8Array | null | undefined): DecodedHashCheck | null {
...
export function encodePong(seq: number, t: number): Uint8Array {
  const buf = new Uint8Array(PING_BYTES);
  const dv = new DataView(buf.buffer);
  buf[0] = PKT.PONG;
  dv.setUint32(1, seq >>> 0, true);
...
export function decodePing(buf: Uint8Array | null | undefined): DecodedPing | null {
  if (!buf || buf.length < PING_BYTES || buf[0] !== PKT.PING) return null;
  const dv = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
  return { seq: dv.getUint32(1, true), t: dv.getUint32(5, true) };
}

export function decodePong(buf: Uint8Array | null | undefined): DecodedPong | null {
...
export function encodeSnapshotRequest(frame: number): Uint8Array {
...
export function decodeSnapshotRequest(
  buf: Uint8Array | null | undefined,
): DecodedSnapshotRequest | null {
...
export function encodeSnapshotChunk(chunk: {
  ...
  const { frame, hash, seq, total, data } = chunk;
...
export function decodeSnapshotChunk(
  buf: Uint8Array | null | undefined,
): DecodedSnapshotChunk | null {
