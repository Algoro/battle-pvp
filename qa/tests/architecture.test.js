...
test("architecture: netcode не зависит от emulator-core/frontend/backend", () => {
  const files = walk(join(ROOT, "netcode"), (p) => p.endsWith(".js") && !p.includes(`${sep}tests${sep}`));
...
test("architecture: backend не зависит от emulator-core/frontend", () => {
  const files = walk(join(ROOT, "backend"), (p) => p.endsWith(".js") && !p.includes(`${sep}tests${sep}`));
...
test("architecture: domain/rom-contract чисты (без внешних зависимостей)", () => {
  const contract = imports(join(ROOT, "emulator-core", "rom-contract.js"));
  assert.deepStrictEqual(contract, [], "rom-contract.js должен быть без импортов");
  const domain = imports(join(ROOT, "emulator-core", "domain.js"));
  for (const spec of domain) {
    assert.ok(spec.includes("rom-contract"), `domain.js может зависеть только от rom-contract, а не "${spec}"`);
  }
});

test("architecture: RollbackSession использует порт Clock, а не Date.now напрямую", () => {
  const src = readFileSync(join(ROOT, "netcode", "rollback", "session.js"), "utf8");
...
test("architecture: backend/domain чист — только внутренние относительные импорты", () => {
  const files = walk(join(ROOT, "backend", "domain"), (p) => p.endsWith(".js"));
  const bad = [];
  for (const f of files) {
    for (const spec of imports(f)) {
...
test("architecture: backend/application зависит только от domain (без persistence/signaling/node)", () => {
  const files = walk(join(ROOT, "backend", "application"), (p) => p.endsWith(".js"));
...
test("architecture: порты самодостаточны (без импортов)", () => {
  assert.deepStrictEqual(imports(join(ROOT, "netcode", "ports.js")), []);
