...
export function wrap(emu: any) {
  return {
    ...
    brain: {
      attacker(mode: any) { emu._attAI = mode; return this; },
      defender(mode: any) { emu._defAI = mode; return this; },
    },
    spawnPrize(id: number, x: number, y: number) { emu.spawnBonus(id, x, y); },
    frame(inputs: any) { return emu.stepFrame(inputs ?? [{ port: 0, buttons: 0 }, { port: 1, buttons: 0 }]); },
  };
}
