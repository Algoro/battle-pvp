...
export class Linker {
  ...
  /**
   * @param {import("./rom-image.ts").RomImage} image
   * @param {object} opts { symbols, free: [{start,end}] }
   */
  constructor(image: any, { symbols = {}, free = [] }: { symbols?: any; free?: any[] } = {}) {
    this.image = image;
  ...
  _claim(at: number, len: number, symbol: any, fill: number): void {
