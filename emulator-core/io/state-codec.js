...
function ppuProps(ppu) {
  return ppu.constructor.JSON_PROPERTIES.filter((p) => !PPU_SKIP.has(p));
}
...
class Writer {
  ...
  u8(v) { this._grow(1); this.buf[this.off++] = v; }
  u16(v) { this._grow(2); this.dv.setUint16(this.off, v, true); this.off += 2; }
  u32(v) { this._grow(4); this.dv.setUint32(this.off, v, true); this.off += 4; }
  f64(v) { this._grow(8); this.dv.setFloat64(this.off, v, true); this.off += 8; }
  bytes(arr) {
    this.u32(arr.length);
  ...
  result() { return this.buf.slice(0, this.off); }
}
...
function encodeProps(w, obj, props) {
  w.u16(props.length);
...
function decodeProps(r, obj, props) {
  const n = r.u16();
  for (let i = 0; i < n; i++) {
    const p = props[i];
    const v = readValue(r);
...
export function encodeState(emu) {
  const w = new Writer();
...
export function decodeState(emu, bytes) {
