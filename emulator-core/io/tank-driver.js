...
export function stagePassable(tileId) {
...
export function canPlace(x, y, field, isPassable = defaultIsPassable) {
...
export function canLead(x, y, dir, field, isPassable = defaultIsPassable) {
  const dx = DX[dir];
  const dy = DY[dir];
  const cx = x + dx;
  const cy = y + dy;
  ...
  function cellPassable(f, px, py, isPass) {
    const tc = tile(px);
    const tr = tile(py);
...
export function stepTank(pos, dir, field, isPassable = defaultIsPassable) {
...
export function canTurn(pos, dir, field, isPassable = defaultIsPassable) {
  return canPlace(pos.x, pos.y, field, isPassable);
}

// ---- простой ИИ защитника ----
// Идёт к цели (x,y) с приоритетом по вертикали/горизонтали.
export function aiDirection(pos, target, field, isPassable = defaultIsPassable) {
...
export function tickTank(pos, dir, field, isPassable = defaultIsPassable) {
