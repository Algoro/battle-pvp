...
export function setStrategyConfig(over = {}) {
  const o = typeof over === "string" ? (PRESETS[over] ?? {}) : over;
  cfg = { ...cfg, ...o };
...
function enemyThreatCells(perc) {
  const set = new Set();
...
function navigateFine(field, tank, goalCell, threatCells, allowBreak) {
...
function allyBlockingLine(perc, from, to, selfIndex) {
...
function bestAlignedFire(perc, tank) {
...
function uEngage(perc, tank, target) {
  ...
  const risk = perc.bullets.filter((b) => b.team === "ATT"
...
function uRushBonus(perc, tank, prize) {
  ...
  if (cfg.threatGatedBonus) {
    ...
    const aliveEnemies = perc.enemies.filter((e) => e.tank.inField).length;
...
function uRetreat(perc, tank, incoming) {
...
function counterShot(perc, tank, ourBusy) {
...
function decideTank(perc, tank, st, frame) {
  ...
  const incoming = perc.bullets.filter((b) => b.team === "ATT"
    && dist(b.cell, cell) <= 12
...
function edgeFire(st, want, ourBusy) {
...
function smoothAndDetour(st, tank, dir, field, goal, baseCell) {
  // анти-застревание
  let moveDir = dir;
...
function anchorSpot(field, base, index) {
...
export function strategyDefense(mem, frame, state = new Map()) {
