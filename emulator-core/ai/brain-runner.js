...
import { DIR_BTN } from "../model/game-view.js";
import { plan, planDefense } from "./tactical-ai.js";
import { scanPlan } from "./scan-ai.js";
import { lookaheadPlan } from "./lookahead-ai.js";
import { strategyDefense } from "./defender-strategy.js";
import { attackerPlan } from "./attacker-strategy.js";

export function runBrain(stateOrMem, prev, mode, role, frame = 0) {
  ...
  if (role === "def") {
    ...
    if (mode === "strategy") {
      const res = strategyDefense(mem, frame, prev);
    ...
    const res = planDefense(mem, frame, prev);
    return { decisions: res.buttons, state: prev ?? res.state ?? new Map() };
  }
