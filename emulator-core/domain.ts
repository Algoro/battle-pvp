// domain.js — DOMAIN SEMANTICS for Battle City (tiles, tank flags, bullets, directions).
//
// Complements rom-contract.js (RAM/ROM addresses): named constants and predicates live here,
// so the code contains no "magic" like `0xa0 | dir`, `hi >= 0x90`, `v <= 0x0f`.
// Single source for the core, AI model, simulator, and patching.
//
// Relative path: ./emulator-core/domain.js
import { BTN } from "./rom-contract.ts";

export { BTN };

// ---------------------------------------------------------------------------
// Directions: 0=Up, 1=Left, 2=Down, 3=Right (matches flag&3 in ROM).
// ---------------------------------------------------------------------------
...
// Logical tanks/ports: 0,1 — DEF ($4016/$4017), 2..7 — ATT (networked/AI).
...
// First pressed direction from the mask (Up, Left, Down, Right) or null.
...
// ---------------------------------------------------------------------------
// Tank flags (ram_tank_flags): 0x00 empty/dead; active 0x90..0xd0;
// "on field" (including animation) 0x80..0xd0; respawn 0xe0..; explosion 0x73.
// ---------------------------------------------------------------------------
export const TANK_MOVING = 0xa0; // moving: 0xa0 | dir
export const TANK_STANDING = 0x88; // turning/idle: 0x88 | dir
export const TANK_EXPLODE = 0x73;
export const TANK_RESPAWN = 0xe0;
export const TANK_RESPAWN_FLAG = 0xf0; // set on DEF respawn
...
// ---------------------------------------------------------------------------
// Tank types (ram_tank_type): 0x80 basic, 0xa0 fast bullets, 0xc0 fast,
// 0xe0 armored. Bit 0x04 — flashing (bonus) enemy.
// ---------------------------------------------------------------------------
export const TANK_TYPE = { BASE: 0x80, FAST_BULLET: 0xa0, FAST_TANK: 0xc0, ARMOR: 0xe0 };

// How many hits a tank withstands by type (see hitsLeft).
export function tankHits(type: number): number {
  if (type & 0x04) return 1;
  if ((type & 0xf0) === TANK_TYPE.ARMOR) return (type & 0x03) + 1;
  return 1;
}
// Approximate tank speed by type (for AI planning).
...
// Bullet property (see bulletProperty): 3 — powered, 1 — fast, 0 — normal.
...
// ---------------------------------------------------------------------------
// Field tiles (collision buffer $0400, values in bytes).
// ---------------------------------------------------------------------------
export const TILE = {
  EMPTY: 0x00,
  STEEL: 0x10,
...
export const BRICK_EXTRA = [0x13, 0x14]; // stage variants
// Bit mask of brick quadrants (matches brickHit logic).
...
// Tanks pass through: 0x00 and road tiles 0x20..0x7F (water/ice/trees see separately).
...
// Blocks a bullet: everything except empty, roads, and destructible brick.
export function blocksBullet(v: number): boolean { return v !== 0 && !(v >= 0x20 && v < 0x80) && !isBrick(v); }
// Brick "health": intact 0x0f — 2, damaged — 1.
export function brickHealth(v: number): number { return v === 0x0f ? 2 : isBrick(v) ? 1 : 0; }

// ---------------------------------------------------------------------------
// Bullet status (ram_bullet_status): 0x40|dir — flying, 0x33 — explosion, 0x01 — busy marker.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// DEF tank upgrade (ram_tank_upgrade): steps 0x20, max 0x60 (3 stars).
// ---------------------------------------------------------------------------
export const UPGRADE = { STEP: 0x20, MAX: 0x60, MAX_STARS: 3 };
// Super-weapon "pistol": N shots on pickup/start.
export const PISTOL_SHOTS = 3;
// Beam half-width in tiles: width = 2*HALF+1 (HALF=1 -> 3 tiles).
