...
import { FIELD, isBrick } from "../model/game-view.js";

function alive(flag) { const hi = flag & 0xf0; return hi >= 0x90 && hi <= 0xd0; }
...
export class EmuObserver {
  ...
  _diff() {
    ...
    const mapB = new Map(b.bullets.map((x) => [x.owner, x]));
    const mapP = new Map(p.bullets.map((x) => [x.owner, x]));
  ...
  reset() { this.events = []; this.prev = null; this.cur = null; }
