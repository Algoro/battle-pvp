...
export const RAM = {
  ...
  // фича friendly-fire-att: битовая маска слотов вражеских пуль (2..7), которые уже
  // покинули «дуло» стрелка (пуля спавнится в его хитбоксе, поэтому самоурон разрешаем
  // только после выхода из дула). Свободная RAM после звукового движка (031C-03FB).
  FF_ATT_CLEARED: 0x03fc, // 1 байт
  // фича enemy-prizes: битовая маска призов (бит id), которые враг может забрать.
  // 0xFF — все разрешены. Читается ROM-хуком подбора; пишется рантаймом из настроек.
  ENEMY_PRIZE_ALLOW: 0x03fd, // 1 байт
};
