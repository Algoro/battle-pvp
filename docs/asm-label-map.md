> 🌐 [English](en/asm-label-map.md) · **Русский**
...
Отдельно, после звукового движка (`$031C–$03FB`) свободны `$03FC–$03FF`; `$03FC`
использует фича `friendly-fire-att`:

| Метка | Адрес | Назначение |
|---|---|---|
| ram_ff_att_cleared | `$03FC` | битовая маска слотов вражеских пуль, вышедших из «дула» стрелка |
| ram_enemy_prize_allow | `$03FD` | битовая маска призов (бит id), которые враг может забрать (`enemy-prizes`; `$FF` — все) |
