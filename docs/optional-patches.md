> 🌐 [English](en/optional-patches.md) · **Русский**
...
- **ROM** (`patches/wrap-borders.ts`): одна запись `$D7CE` `A9 11` → `A9 00` — рамка
  уровня (сталь `$11`) больше не заливается ни в коллизию, ни в nametable.
- **JS-рантайм** (`features/wrap-borders.ts`): игровая зона 26×26 становится тором —
  объект, вышедший за шов, появляется с противоположной стороны. Перенос по факту
  перехода «внутри → снаружи»; занятая противоположная сторона — танк упирается, пуля
  гаснет. Детерминированно, в `postFrame`.
- Подробности — `docs/wrap-borders.md`.
...
- **Рендер**: 2D — блендинг спрайтов танка/пуль прямо в пиксельный буфер PPU (render-хук); 3D (`topdown-3d`/`meine-tank`) —
