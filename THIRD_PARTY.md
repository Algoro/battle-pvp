...
## Ресурспак Faithful 32x (выборочные текстуры для рендерера `meine-tank`)
- Источник: **Faithful 32x** — https://faithfulpack.net/ (лицензия Faithful v3, файл
  `frontend/public/textures/meine-tank/LICENSE.txt` включён без изменений).
- В репозиторий попадает **только курированный набор** (см.
  `frontend/src/render/drivers/meine-tank/textures/manifest.ts`); полный пак не копируется.
  Извлечение — `scripts/build-meine-tank-textures.mjs` (сам `.zip` в `.gitignore`).
- Условия Faithful: атрибуция и ссылка (этот раздел), сохранение `LICENSE.txt`, запрет
  монетизации контента с их текстурами и запрет выдавать проект за официальный. Проект —
  бесплатная неофициальная фанатская работа; условия соблюдаются.
- Текстуры — производная/немодифицированная часть пака; права принадлежат авторам Faithful.
