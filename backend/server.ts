...
import { RelayServer } from "./signaling/relay.ts";
import { Store } from "./persistence/store.ts";
import { SqliteChatRepository } from "./persistence/chat-repository.ts";
import { LobbyManager } from "./domain/lobby.ts";
import { startMatch } from "./application/match-lifecycle.ts";
...
export function createApp({ dbPath }: { dbPath?: string } = {}): App {
  ...
  const http = createServer((req, res) =>
    void handleHttp(req, res, { store, rooms, matchmaker, lobbies, chat }),
  );
  ...
  return {
    ...
    close() {
      ...
      wss.close();
      return new Promise<void>((r) => http.close(() => r()));
    },
  };
}
...
async function handleHttp(
  ...
  if (req.method === "POST" && path === "/matchmake") {
    ...
    if ("queued" in res2) return json(res, 202, { queued: true });
  ...
  if (req.method === "POST" && path === "/lobbies") {
    ...
    const lobby = ctx.lobbies.create({
      ...
      name: body.lobbyName || `${playerName} — игра`,
    ...
    const r = lobby.join({
      ...
      sessionId: null,
      socket: null,
      fingerprint: body.cartridgeFingerprint,
    });
    if (!r.ok) {
