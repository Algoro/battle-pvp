// store.js — персистентность матчей/игроков на SQLite (node:sqlite).
// Без Docker/root — прямой Node-процесс + файл БД. Относительный путь к БД.
//
// Относительный путь: ./backend/persistence/store.js
...
export class Store {
  /**
   * @param {string} dbPath  — путь к файлу БД (относительный от ./backend)
   */
  constructor(dbPath = join(DB_DIR, "battlecity.sqlite")) {
